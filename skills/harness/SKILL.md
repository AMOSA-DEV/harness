...
하네스 생성·수정 시 다음 실행 품질 기준을 반드시 확인한다: 사용자 숙련도 감지, context 자동 채우기, `_workspace/` 분리와 보존, 부분 재실행, 역할별 파일 출력, 검증 루프, 트리거 near-miss, 세션 파일 실제 갱신.

## `_workspace/` 표준 구조

새 하네스는 `_workspace/` 아래 산출물을 목적별로 분리한다.

| 경로 | 목적 |
|---|---|
| `_workspace/audit/` | 하네스 감사 결과, 구조 검증, QA 리포트 |
| `_workspace/machine/` | 부분 재실행, 재작업, 다음 역할 입력에 필요한 MACHINE용 산출물 |
| `_workspace/human/` | HUMAN이 검토할 최종 산출물, 완료 보고서, 승인용 문서 |

기존 하네스에 `_workspace/{phase}_{role}_{artifact}.md` 같은 루트 산출물이 있으면 즉시 삭제하거나 강제 이동하지 않는다. 새로 생성하는 산출물부터 위 구조를 사용하고, 마이그레이션이 필요하면 감사 결과에 기존 경로와 새 권장 경로를 함께 남긴다.

새 실행으로 기존 workspace를 보존해야 할 때는 `_workspace_archive/YYYYMMDD_HHMMSS/` 또는 프로젝트가 정한 동등한 timestamp 백업 경로로 `_workspace/` 전체를 보존한다. 백업 전에는 삭제하지 않고, 백업 위치와 재사용할 핵심 파일을 `_workspace/audit/` 감사 메모나 `HANDOFF.md`에 기록한다.
...
감사 결과는 `_workspace/audit/` 아래에 남기며, 최소한 다음을 포함한다:
...
9. 파일 기반 산출물 경로를 `_workspace/audit/`, `_workspace/machine/`, `_workspace/human/` 기준으로 정한다.
...
4. 역할 간 데이터 전달 방식을 정한다: 파일 기반 `_workspace/machine/`, 런타임 메시지, 작업 상태, 반환 요약 중 무엇을 쓰는지 명시한다.
...
- `_workspace/` 목적별 폴더, 백업 계획, 부분 재실행, 새 실행 분기가 오케스트레이터 Phase 0에 있는가
...
1. 현황 감사 결과를 `_workspace/audit/harness_audit.md`에 남긴다.
